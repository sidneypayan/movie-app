...
export function MoviesContextProvider({ children }) {
	...
	useEffect(() => {
		...
		setDisplayMovies(apiMovies)
	}, [])

	useEffect(() => {
		function displayRegardingLocation() {
			if (location.pathname === '/') {
				setDisplayMovies(apiMovies)
			} else if (location.pathname === '/films-a-voir') {
				setDisplayMovies(favoriteMovies)
			} else if (location.pathname === '/films-vus') {
				setDisplayMovies(watchedMovies)
			}
		}
		displayRegardingLocation()
	}, [location.pathname])
